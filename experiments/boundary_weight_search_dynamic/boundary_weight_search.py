...
for i, w_bc in enumerate(weights):
    ...
    if i > 400:
        ...
        if i % 50 == 0:
            ...
            np.savetxt(f"weights2d_dynamic_{i}.csv", data, delimiter="," ,header="Idx,w_bc,min-bc-loss,min-total-loss", comments='')
 
data = np.column_stack([idx, w_bc_list, min_bc_loss, min_total_loss])
np.savetxt("weights2d_dynamic.csv", data, delimiter=",",  header="Idx,w_bc,min-bc-loss,min-total-loss", comments='')


# %%
import matplotlib.pyplot as plt

data = np.genfromtxt('weights2d_dynamic_400.csv', delimiter=',', skip_header=True)
