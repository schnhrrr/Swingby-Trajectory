...
class TrajectoryOptimizer:
    ...
    def __init__(self, model, ao_xyzgm, t_colloc, t_total, r0, r1, opt_lbfgs, opt_adam, n_lbfgs=150, n_adam=0, w_physics=1):
    ...
    def plot_trajectory(self):
        ...
        # TODO: Add forces to trajectory plot using quiver

        x, y, z = self.r_plt[:,0], self.r_plt[:,1], self.r_plt[:,2]

        self.fig_traj, self.ax_traj = plt.subplots(2,2, figsize=(10,8))
        self.fig_traj.delaxes(self.ax_traj[1,1])
        self.plot_masses()

        self.ax_traj[0,0].scatter(-1,-1,color='r',marker='o',label=f'r(t=0)=(-1,-1,-1)')
        self.ax_traj[0,0].scatter(1,1,color='r',marker='x',label=f'r(t=0)=(1,1,1)')
    ...
    def plot_masses(self):
        ...
        for i, ao in enumerate(self.ao_plt):
            self.ax_traj[0,0].scatter(ao[0], ao[1], s=ao[3]*300, c='k', marker='o')
            self.ax_traj[0,0].text(ao[0], ao[1] + 0.1, f'GM$_{i}$')
            self.ax_traj[0,1].scatter(ao[0], ao[2], s=ao[3]*300, c='k', marker='o')
            self.ax_traj[0,1].text(ao[0], ao[2] + 0.1, f'GM$_{i}$')
            self.ax_traj[1,0].scatter(ao[1], ao[2], s=ao[3]*300, c='k', marker='o')
            self.ax_traj[1,0].text(ao[1], ao[2] + 0.1, f'GM$_{i}$')
...
m0 = 1.
ao_xyzgm = [[-0.5, -1., 0., 0.2],  # astronomic objects: x, y, z gravitational mass
            [-0.2, 0.4, 0., 0.4],
            [ 0.8, 0.3, 0., 0.3]]
...
obj = TrajectoryOptimizer(pinn, ao_xyzgm, t_colloc, t_total, r0, r1, optimizer_lbfgs, optimizer_adam, w_physics=7, n_adam=200)
