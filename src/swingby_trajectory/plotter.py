...
class TrajectoryPlotter:
    ...
    def _generate_fig_name(self):
        if self.prefix:
            ...
            return (
                "_".join([f"{k.strip()}" for k, _ in self.experiments.items()])
                + str(self.dim)
                + "d"
            )
    ...
    def plot_thrust(self):
        ...
        fig.tight_layout()
        fig.savefig(
            f"{self._generate_fig_name()}_thrust.pdf",
            bbox_inches="tight",
            pad_inches=0.05,
        )
        plt.show()
    ...
    def plot_gravity(self):
        ...
        fig.tight_layout()
        fig.savefig(
            f"{self._generate_fig_name()}_gravity.pdf",
            bbox_inches="tight",
            pad_inches=0.05,
        )
        plt.show()
    ...
    def _plot_traj_3d_projection(self, plot_quiver=True, plot_legend=True):
        ...
        plt.show()
        fig.savefig(
            f"{self._generate_fig_name()}_traj2d.pdf",
            bbox_inches="tight",
            pad_inches=0.05,
        )
    ...
    def plot_traj_2d(self, plot_quiver=True, plot_legend=True):
        ...
        for label, exp in self.experiments.items():
            ...
            result, color, linestyle, quiver_scale = (
                exp["result"],
                exp["color"],
                exp["linestyle"],
                exp["quiver_scale"],
    ...
    def plot_traj_3d(self, plot_quiver=True, plot_legend=True):
        ...
        for label, exp in self.experiments.items():
            ...
            r_q, G_q, T_q = self._get_quiver_data(res)
            ax.plot3D(
                res.r[:, 0], res.r[:, 1], res.r[:, 2], label=label, color=exp["color"]
            )
            if plot_quiver:
                ax.quiver(
                    r_q[:, 0],
        ...
        self.fig_3d.savefig(
            f"{self._generate_fig_name()}_traj3d.pdf",
            bbox_inches="tight",
            pad_inches=0.05,
        )
        plt.show()
    ...
    def plot_loss(self, x_lim=None):
        ...
        ax.legend()
        fig.savefig(
            f"{self._generate_fig_name()}_loss.pdf",
            bbox_inches="tight",
            pad_inches=0.05,
        )
        plt.show()
