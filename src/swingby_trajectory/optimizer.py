...
class TrajectoryOptimizer:
        ...
        self.v = torch.stack(
            [
                torch.autograd.grad(
                    self.r[:, i],
                    self.t,
                    grad_outputs=torch.ones_like(self.r[:, i]),
                    create_graph=True,
                )[0].squeeze(-1)
                for i in range(self.dims)
            ],
            ...
            torch.stack(
                [
                    torch.autograd.grad(
                        self.v[:, i],
                        self.t,
                        grad_outputs=torch.ones_like(self.v[:, i]),
                        create_graph=True,
                    )[0].squeeze(-1)
                    for i in range(self.dims)
                ],
    ...
    def _train_model(self):
        ...
        if isinstance(self.adam, functools.partial):
            adam_optimizer = self.adam(
                self.model.parameters(), **getattr(self.adam, "keywords", {})
            )
        if isinstance(self.lbfgs, functools.partial):
            lbfgs_optimizer = self.lbfgs(
                self.model.parameters(), **getattr(self.lbfgs, "keywords", {})
            )
        prev_loss = float("inf")
        ...
        for i in tqdm(range(self.n_adam + self.n_lbfgs)):
            ...
            if (loss_change < 1e-5) & (i > self.n_adam):
                print(
                    f"Convergence reached at iteration {i} with loss change {loss_change:.6f}."
                )
                break
    ...
    def _closure(self):
        ...
        if self.w_bc > 0:
            self.loss_bc = (
                (
                    torch.mean((self.r[0] - self.r0) ** 2)
                    + torch.mean((self.r[-1] - self.rN) ** 2)
                )
                / 2
                * self.w_bc
            )
