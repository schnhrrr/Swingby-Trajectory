...
def plotTrajectory(r):
    ...
    i = 1
    for xtemp, ytemp, _, gmtemp in ao_xygm:
        ax[0,0].scatter(xtemp, ytemp, s=gmtemp*500, c='k', marker='o')
    ...
    ax[0,0].legend()
    ax[0,1].plot(x,z, label='xz')
    ax[0,1].set_xlabel(r'$x$')
    ax[0,1].set_ylabel(r'$z$')
    #ax[0,1].set_ylim((-1,1))
    ax[1,0].plot(y,z, label='yz')
...
def ode(t, r):
    ...
    return ode_x.view(-1,1), ode_y.view(-1,1), z.unsqueeze(1)
...
r0          = torch.tensor([[-1.,-1.,0.]]) # start point
r1          = torch.tensor([[1.,1.,0.]]) # end point
m0          = 1.
T           = torch.tensor(1.0, requires_grad=True) # end time

ao_xygm     =  [[-0.5,  -1., 0., 0.5],  # astronomic objects: x, y, z gravitational mass
                [-0.2,  0.4, 0., 1.0],
                [ 0.8,  0.3, 0., 0.5]]
