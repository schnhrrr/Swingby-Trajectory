...
class PINN(nn.Module):
    ...
    def __init__(self, N_INPUT, N_OUTPUT, N_NEURONS, N_LAYERS, A):
        super().__init__()
    ...
    def forward(self, t):
        ...
        g = torch.cat([2*t-1, 2*t-1,-(2*t-1)*self.A], dim=1)
...
class TrajectoryOptimizer:
    ...
    def plot_trajectory2d(self):
        ...
        self.fig_traj.suptitle(f'A = {self.model.A}')
        
        # (x,y)
        self.ax_traj[0,0].scatter(-1,-1,color='r',marker='o',label=r'$r(t=0)=(-1,-1,A)$')
        self.ax_traj[0,0].scatter(1,1,color='r',marker='x',label=r'$r(t=1)=(1,1,-A)$')
        self.ax_traj[0,0].plot(self.x, self.y, label='Trajectory')
        self.ax_traj[0,0].set_xlabel('x')
        self.ax_traj[0,0].set_ylabel('y')

        # (x,z)
        self.ax_traj[0,1].scatter(-1,self.model.A,color='r',marker='o',label=f'r(t=0)=(-1,-1,A)')
        self.ax_traj[0,1].scatter(1,-self.model.A,color='r',marker='x',label=f'r(t=0)=(1,1,-A)')
        self.ax_traj[0,1].plot(self.x, self.z)
        self.ax_traj[0,1].set_xlabel('x')
        self.ax_traj[0,1].set_ylabel('z')

        # (y,z)
        self.ax_traj[1,0].scatter(-1,self.model.A,color='r',marker='o',label=f'r(t=0)=(-1,-1,-1)')
        self.ax_traj[1,0].scatter(1,-self.model.A,color='r',marker='x',label=f'r(t=0)=(1,1,1)')
    ...
    def plot_trajectory3d(self):
        ...
        self.ax3d.scatter(-1,-1,self.model.A, marker='o', color='r', label=f'r(0)=(-1,-1,0)')
        self.ax3d.scatter(1,1,-self.model.A, marker='x', color='r', label=f'r(1)=(1,1,0)')  
...
#A = np.sin(np.deg2rad(23)) * np.sqrt(2) = 0.553
A = 0.553  # constant to shift inital and final condition in z-direction
r0 = torch.tensor([[-1., -1., +A]])
r1 = torch.tensor([[1., 1., -A]])
